...
ON CONFLICT (question_id) DO UPDATE SET
    ...
    -- COALESCE, not a blind overwrite: if this run couldn't find/upload the
    -- local image file (image_key comes through as NULL in that case -- see
    -- prepare_row/the upload loop in main()), keep whatever image_key/
    -- image_meta the row already had rather than wiping out a value from a
    -- previous successful run. A script that's "safe to re-run" must never
    -- regress previously-good data just because a local path didn't resolve
    -- on this particular machine/run.
    image_key = COALESCE(EXCLUDED.image_key, questions.image_key),
    image_meta = COALESCE(EXCLUDED.image_meta, questions.image_meta),
...
def main():
    ...
    for q in with_image:
        ...
        if ref:
            ...
            candidate_key = f"questions/{filename}"
            local_path = os.path.join(QUESTION_IMAGES_DIR, filename)
            if os.path.exists(local_path):
                s3.upload_file(local_path, MINIO_BUCKET, candidate_key)
                image_key = candidate_key
                uploaded += 1
            elif object_exists(s3, candidate_key):
                # Local file is missing (moved/deleted since the last run),
                # but the object is already sitting in MinIO from a previous
                # successful upload -- trust it rather than treating this as
                # a failure. This is what actually lets a stale/misconfigured
                # QUESTION_IMAGES_DIR self-heal on a later run once it's
                # fixed, and keeps this run from needing the local file at
                # all if the object's already there.
                image_key = candidate_key
            else:
                upload_failures.append((q["question_id"], local_path))
        rows.append(prepare_row(q, image_key=image_key))

    print(f"Uploaded {uploaded} images to bucket '{MINIO_BUCKET}'")
    if upload_failures:
        print(
            f"WARNING: {len(upload_failures)} questions referenced an image file that "
            f"wasn't found locally -- image_key left as-is in the DB for these (COALESCE "
            f"in the upsert preserves any value from a prior successful run instead of "
            f"nulling it out). QUESTION_IMAGES_DIR is currently: {QUESTION_IMAGES_DIR}\n"
            f"First few missing paths checked:"
        )
        for qid, path in upload_failures[:10]:
            print(f"    {qid}: {path}")
