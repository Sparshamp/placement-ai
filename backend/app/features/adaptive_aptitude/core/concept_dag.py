...
class ConceptDAG:
    ...
    def __init__(self):
        ...
        # Lazy caches for get_concepts_by_subject/get_concepts_by_practice_category --
        # these scan self.nodes, which is static once the DAG is built (see
        # add_concept/add_prerequisite's cache invalidation above). Cheap
        # even uncached (~0.09ms at 2,539 nodes, see the profiling note in
        # experiments/selectors/_common.py), but every select() call hit
        # this, so caching is free correctness-wise and removes it as a
        # contributor entirely.
        self._concepts_by_subject_cache: Optional[Dict[str, List[str]]] = None
        self._concepts_by_category_cache: Optional[Dict[str, List[str]]] = None
    ...
    def add_prerequisite(self, prereq_id: str, dependent_id: str):
        ...
        if prereq_id in self.nodes and dependent_id in self.nodes:
            ...
            # prerequisites don't affect subject/category membership lists,
            # so no need to invalidate the scope cache here.

    def _invalidate_scope_cache(self):
        self._concepts_by_subject_cache = None
        self._concepts_by_category_cache = None
    ...
    def get_concepts_by_subject(self, subject: str) -> List[str]:
        if self._concepts_by_subject_cache is None:
            cache: Dict[str, List[str]] = {}
            for nid, n in self.nodes.items():
                cache.setdefault(n.subject, []).append(nid)
            self._concepts_by_subject_cache = cache
        return self._concepts_by_subject_cache.get(subject, [])
    ...
    def get_concepts_by_practice_category(self, practice_category: str) -> List[str]:
        ...
        session-scoping key."""
        if self._concepts_by_category_cache is None:
            cache: Dict[str, List[str]] = {}
            for nid, n in self.nodes.items():
                if n.practice_category is not None:
                    cache.setdefault(n.practice_category, []).append(nid)
            self._concepts_by_category_cache = cache
        return self._concepts_by_category_cache.get(practice_category, [])
