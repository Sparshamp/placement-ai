...
def image_url_for_key(image_key):
    # pandas.read_sql_query commonly returns SQL NULL as float('nan') even
    # for a TEXT column, not None -- and bool(float('nan')) is True in
    # Python, so a plain `if not image_key` never catches it. Every
    # non-image question was silently getting image_url =
    # "http://.../question-images/nan" (the literal string "nan") instead
    # of None, which the frontend then tried to render as a real image.
    # pd.isna() correctly identifies None, NaN, and pd.NA as missing.
    if pd.isna(image_key) or not image_key:
        return None
